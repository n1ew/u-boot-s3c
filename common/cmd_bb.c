...
int do_bb(cmd_tbl_t * cmdtp, int flag, int argc, char *argv[])
{
	...
	if (strcmp(cmd, "qtopia") == 0)
	{
		...
		if(run_command ("tftp c0008000 zImage_qtopia", flag) < 0)
			goto usage;
		if(run_command ("nand erase 40000 3c0000", flag) < 0)
			goto usage;
		if(run_command ("nand write c0008000 40000 3c0000", flag) < 0)
			goto usage;	
	...
	{
		if(run_command ("nand read c0008000 40000 3c0000", flag) < 0)
			goto usage;
